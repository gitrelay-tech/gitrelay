...
function resolveLlmTargetAuto(
  ...
  return {
    error:
      "No LLM API key configured. Set GITRELAY_QUICK_LLM and the matching key(s), or leave GITRELAY_QUICK_LLM unset (auto) and set one of: XAI_API_KEY, OPENROUTER_API_KEY, OPENAI_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY.",
  };
}

function resolveLlmTarget(): LlmTarget | { error: string } {
  const modeRaw = process.env.GITRELAY_QUICK_LLM?.trim().toLowerCase() ?? "";
  ...
  if (!valid.has(mode)) {
    return {
      error:
        "Invalid GITRELAY_QUICK_LLM. Use grok, openrouter, openai, google, or auto.",
    };
  }
  ...
  switch (explicitMode) {
    case "grok":
      if (!xaiKey) {
        return {
          error:
            "GITRELAY_QUICK_LLM=grok requires XAI_API_KEY in .env.local.",
        };
      }
    ...
    case "openrouter":
      if (!openRouterKey) {
        return {
          error:
            "GITRELAY_QUICK_LLM=openrouter requires OPENROUTER_API_KEY in .env.local.",
        };
      }
    ...
    case "openai":
      if (!openAiKey) {
        return {
          error:
            "GITRELAY_QUICK_LLM=openai requires OPENAI_API_KEY in .env.local.",
        };
      }
    ...
    case "google":
      if (!googleKey) {
        return {
          error:
            "GITRELAY_QUICK_LLM=google requires GOOGLE_GENERATIVE_AI_API_KEY in .env.local.",
        };
      }
